...
```{r}
setwd('D:\\Users\\seyma\\Documents\\GitHub\\fall21-seymacakir\\Project')
set.seed(448)
...
df = long_train[long_train$hour==j]
rpart_matrix = data.table(t(1:72))

for (i in unique(df$id)){
    # select the data for representation
    data=df[id==i]
    # set parameters and represent data
    tree_fit=rpart(value~time,data,control=rpart.control(maxdepth=3))
    data[,tree_rep:=predict(tree_fit,data)]
    # change actual variables with representation
    rpart_matrix = rbind(rpart_matrix, t(data$tree_rep))
}

rpart_matrix = rpart_matrix[2:nrow(rpart_matrix),]
rpart_dist_euc=as.matrix(dist(rpart_matrix))
diag(rpart_dist_euc)=2*max(rpart_dist_euc)
train_class = train_data[hour==j]$system_direction
...
results = rbindlist(get_acc(rpart_dist_euc,3,10,k_levels,train_class)[[1]])
...
print(j)
pred = c(apply(rpart_dist_euc,1,order)[(nrow(rpart_dist_euc)-1),ncol(rpart_dist_euc)],nrow(rpart_dist_euc))
print(nn_classify_cv(rpart_dist_euc,train_class,test_indices = pred,klevel)$prediction$predicted[-1])
...
scaled_net = (data$net - mean(data$net))/ sd(data$net)
data[, scaled_net := scaled_net]
lm_scaled=lm(scaled_net~trnd+w_day+mon+as.factor(hour),data)
summary(lm_scaled)

tmp=copy(data)
tmp[,actual:=scaled_net]
tmp[,predicted:=predict(lm_scaled,tmp)]
#head(tmp)
